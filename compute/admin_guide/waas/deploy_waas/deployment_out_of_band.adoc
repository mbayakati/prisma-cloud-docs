...
It is possible to mirror traffic for out-of-band inspection using Prisma Cloud Compute Defenders without requiring any additional configuration by using AWS traffic mirroring.
...
. (Optional) Enable *VPC traffic mirroring* to allow the mirrored traffic to flow from the configured source instance to a Prisma Cloud Observer instance.
+
NOTE: Ports cannot be auto-detected when using *VPC traffic mirroring* because no agent is directly deployed on the source workload and the traffic is routed to the Prisma Cloud Observer via the CSP's traffic mirroring service.
+
